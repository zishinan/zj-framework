...
CREATE TABLE `CURSTOMER_ORDER` (
    `ORDER_ID` bigint NOT NULL COMMENT '工单单号',
    `MID` bigint NOT NULL COMMENT '乐号',
    `CATEGORY_ID` bigint NOT NULL COMMENT '工单类型',
    `SERISE_ID` bigint NOT NULL COMMENT '问题分类',
...
CREATE TABLE `SETTING_SERISE` (
    ...
    `MEI_QIA_GROUP` varchar ( 255 ) NOT NULL COMMENT '美恰客服组',
    `MEI_QIA_ID` varchar ( 255 ) NOT NULL COMMENT '美恰客服id'
  ,  PRIMARY KEY (`ID`)
);
...
CREATE TABLE `CUSTOMER_FAQ` (
    ...
    `CATEGORY_ID` bigint NOT NULL COMMENT '类型id',
    `SERISE_ID` bigint NOT NULL COMMENT '分类id',
...
CREATE TABLE `CURSTOMER_ORDER_EXTR` (
    `ORDER_ID` bigint NOT NULL COMMENT '工单单号',
...
CREATE TABLE `SETTING_ORDER_SERISE_EXTR` (
    ...
    `NAME` int ( 11 ) NOT NULL COMMENT '字段名称',
